...
public class AgentsController {
	...
	public String addAgent(@ModelAttribute("addAgentForm") AgentBean agent,
			Model model) {
		if (agent != null) {
			...
			if (agentId != 0) {
				...
				model.addAttribute("msg",
						"Successfully added a new agent!<br/>Agent Id : "
								+ agentId);
			} else {
				model.addAttribute("result", "error");
				model.addAttribute("msg",
						"An error has occurred while adding agent.");
			}
	...
	public String viewAgent(@ModelAttribute("viewAgentForm") AgentBean agent,
			Model model) {
		if (agent != null) {
	...
	public String updateAgent(@ModelAttribute("viewAgentForm") AgentBean agent,
			Model model) {
		if (agent != null) {
			...
			if (agentInfo == null) {
				model.addAttribute("result", "error");
				model.addAttribute("msg",
						"An error occurred while updating the agent information!");
			} else {
				model.addAttribute("result", "success");
				model.addAttribute("agentExist", Boolean.valueOf(true));
				model.addAttribute("viewAgentForm", agentInfo);
	...
	public String searchAgent(@ModelAttribute("viewAgentForm") AgentBean agent,
			Model model) {
		if (agent != null) {
			int agentId = agent.getAgentId();
			if (agentId != 0) {
	...
	public String deleteAgent(@ModelAttribute("viewAgentForm") AgentBean agent,
			Model model) {
		if (agent != null) {
			boolean deleteSuccess = agentsService.deleteAgent(agent
					.getAgentId());
			if (!deleteSuccess) {
				model.addAttribute("result", "error");
				model.addAttribute("msg",
						"An error occurred while deleting agent information!");
			} else {
				model.addAttribute("result", "success");
				model.addAttribute("msg",
						"Agent information deleted successfully!");
			}
		}
		return "deleteAgent";
	}
